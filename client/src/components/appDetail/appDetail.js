    ...
    render(){
        ...
        return(
            <div className='wrapper'>
                        ...
                        {this.state.data.map(value=>{
                                            ...
                                            value.platforms.map(data=>
                                                data.attributes.map(platform=>
                                                    <React.Fragment key={platform.id} >
                                                        ...
                                                        <div className={`selectPlatformbox ${this.state.platformId.filter(obj=>obj==platform.id).length||info.platform_ids.filter(value=>value==platform.id).length?'selected':''}`} style={{display:'inline-block',width:'122px'}} onClick={(e)=>this.selectPlatform(platform.id)}>
                            ...
                            <div className="createBuild view-brak-bx">
                                <button  type="button" className="view-proto-btn">
                                    <span className="desktop"><a href={`http://localhost:3000/features/${info.title}`} style={{textDecoration:'none',color:'#f02397'}}>Customize with an Expert</a> </span>
                ...
                <div className='commonPopUp higher-zindex active'>
                    <div  className="popOverlay"></div>
                    <div className='popHolder loginRegister'>
                    <div>
                    <button className='cross' onClick={event =>  window.location.href='/'} style={{float:'right'}}>X</button>
                </div>
                    <div className='loginRegisterRight' >
            <div  className="heading-appdetail" style={{textAlign:'left'}}>
                   ...
                   {this.state.nextStep?<div  className="authHeading">  <span className="authHeading-main">Instant quote</span><span className="authHeading-sub">for your product</span></div>:<div  className="authHeading"><span  className="authHeading-main" style={{fontWeight:'bolder'}}>Bring your idea to life</span>
                    ...
                    <form name='registerForm'>
                        <ul className={`signupSteps ${this.state.nextStep?'':'activeStep'}`}>
                            <li>
                                <span  className="field-labels" style={{textAlign:'left'}}>Sign up to builder</span>
